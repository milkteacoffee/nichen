...
> 最后更新：2026-09-26 · 代码版本 **v0.17.0（药铺商店 + 战斗表现改造 + 门口补路）** · 前一里程碑 v0.16.0
> 本次增量（详见 §8 顶部）—— **两处真缺陷 + 两处表现改造**：
> ①**药铺商店**：原来药铺柜台只挂对话、全游戏只有刘记杂货一家商店。
>   修法是"沈伯无话可说时开店"（改 `shenBo` 的兜底分支），**不是**把柜台动作改成商店
>   —— 沈伯站在柜台之后、玩家够不到他本人，拆开会把 M0/M1 主线对话整条吞掉。
> ②**战斗倒计时**：30s → **15s**、**进度条删除**、改成顶栏正中一个大号数字。
> ③**战斗胜利 → 纯文字战利品浮层**：新增 `G.game.loot()` 通道（不复用 toast ——
>   toast 是带框的板）；11px、无框、约 3 秒、最多 5 条、HUD 下方靠右；
>   特殊物品金色 + 呼吸辉光；`toast('战斗胜利')` 已删。
> ④**门口补路**：铺完 `md.paths` 后把**每栋建筑门口那一格**铺成 `path`（只补一格，
>   不硬接长引道 —— 长引道会横穿草地树林，像凭空画出来的线）。
> ⑤契约 **32 → 35 条**（`shop.apothecary` / `battle.hud` / `door.path`），
>   **反例验证 3 组全部命中**（其中"去掉门口补路"报出 3 栋建筑门口不是路，
>   **证实用户反馈的问题真实存在**）。
> **下一步见 §8 顶部「下一步」。**
> v0.16.0（文字遮挡 + 建筑换图）与更早版本见 §8 下方小节。
