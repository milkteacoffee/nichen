...
> 最后更新：2026-09-25 · 代码版本 **v0.0.2（天道意志接入）** · 前版 v0.5.2 commit `814d8f2`
...
| 当前进度 | **M0 主线全通 + 天道意志已接入**（问卦/低语/死亡拦魂；Ollama 远程 + 模板兜底），M1 未开工 |
| 回归状态 | smoke 通过 · playthrough 通过 · rebirth 通过（仙力 +386）· 浏览器实测问卦/拦魂/低语通过 |
...
| `tiandao.js` | 360 | **天道意志**（v0.0.2 新增）：配置/感应五阶段/世界状态包/OpenAI 兼容调用/JSON 校验/模板兜底/问卦/注视低语/DOM 输入框/菜单与设置面板（`G.TianDao`） |
