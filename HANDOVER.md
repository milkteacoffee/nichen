...
> 最后更新：2026-09-26 · 代码版本 **v0.7.0（飞升台 + 区域可见性）** · 前一里程碑 v0.6.0 commit `d7230e5`
> 本次增量（**P4 收口**，详见 §8）：
> ①**区域物件可见性**：秘境裂隙与界门此前**只登记不绘制** —— 地图上什么都不出现，
>   玩家唯一的线索是走到正面冒出一个小三角（等于把入口藏了）。现补
>   `art.js: A.rift / A.worldgate` 与 `explore._drawSpecial` 的两个绘制分支；
> ②**副本入口面板**（缺口 U6）：区域裂隙点了直接开**那一处**秘境的面板，不再一律跳通用枢纽；
>   并修掉"裂隙与枢纽来自两条独立随机序列"（新发现缺口 G20）；
> ③**轮回殿·飞升台**（缺口 U2 的正式入口）：选下一世主界、调各界难度、显示碎片与称号；
> ④**称号展示 + 飞升/道界/地狱成就**（缺口 U3）：角色面板 / 死亡结算 / 飞升台三处可见，
>   成就补 A6–A9（`hit(save, meta)`）；
> ⑤**副本序列种子化**（缺口 U8）：同一世同一界的秘境序列可复现。
> `www/js` 12.8k → **13.5k 行**，`tools/` 4.3k → **5.6k 行**（新增 4 条契约）。
...
| 代码量 | `www/js` ≈ 13.5k 行；`tools/` ≈ 5.6k 行（无头测试与审查工具） |
| 当前进度 | **M0 主线全通 + 天道意志 + 19境/四界/15副本 + 签名秘术效果 + 20 Boss 立绘 + NPC 精灵/立绘全量替换 + 四界 28 区域层/建筑可进/副本入口随机落位 + 地狱难度体系（三碎片开道界）+ 区域裂隙与界门可见 + 副本入口面板 + 轮回殿飞升台（选下一世主界/调界域难度/碎片与称号）+ 飞升·道界·地狱成就**（圣术面板% / 神术战斗被动 / 仙术每场一次主动，数值随品阶），**道界内容（道则回廊三试炼）未开工** |
| 回归状态 | smoke · playthrough · rebirth · **dungeon-run（凡→灵→仙）** · **secret-test（秘术效果）** 全过；smoke 新增 4 条契约（`region.visual` / `dungeon.entrance` / `ascend.hall` / `achieve`，均已做反例验证）；浏览器实测难度页/秘境枢纽/战斗/秘术面板通过 |
...
| 文件 | 行数 | 职责 |
|---|---|---|
| `art.js` | 2474 | **全部程序化美术**。地面纹理、建筑、家具、装饰、精灵、立绘、图标、**区域物件（秘境裂隙 / 界门）**；三级缓存（`cached` / `G.Sprites` / `G.UI`）都在这里定义 |
...
| `explore.js` | 984 | **探索引擎**（镇/山/洞/室内/区域通用）：寻路、交互、遭遇、**HUD**、**区域物件绘制（宝箱 / Boss / 裂隙 / 界门）** |
...
| `regiongen.js` | 391 | **四界 28 区域生成**：由 `data/regions.js` 的紧凑规格确定性产出完整地图并注册进 `G.Data.maps`；场景工厂 + 建筑门路由 + **界门面板/传送（`openGate` / `travelTo`）** |
| `interiorgen.js` | 231 | **建筑内部程序化生成**：9 类模板（民居/杂货/客栈/药铺/铁匠铺/丹房/神殿/大殿/楼阁/守卫所），`int.<regionId>.<buildingId>` 惰性生成 |
| `overlays.js` | 182 | 覆盖层公共件：暗底、对话（立绘+台词）、**角色面板（含称号行）** |
| `tiandao.js` | 425 | **天道意志**（v0.0.2 新增）：配置/感应五阶段/世界状态包/OpenAI 兼容调用/JSON 校验/模板兜底/问卦/注视低语/DOM 输入框/**菜单（角色·秘境·界域难度·天道设置）与设置面板**（`G.TianDao`，含 `isMenuOverlay` 路由白名单） |
| `assets.js` | 107 | 素材登记/查询/加载（`G.Assets.img/register/load`） |
| `storage.js` | — | localStorage 存档 **v2**（`nichen_meta` / `nichen_save`，各带 `_bak`；含 v1→v2 迁移） |
...
| `death.js` | 255 | 死亡结算：死因/享年/走马灯/仙力明细/**称号** → 写入轮回档案 + **天道跨世记忆**（v0.0.2，memory 最近 30 世、watchTotal） |
| `heaven.js` | 150 | **天道拦魂**（v0.0.2 新增）：死亡后全屏星河场景，≤3 轮模型对话（预设/自由应答）→ 死亡结算 |
...
> ✅ 以下数字已于 **2026-09-26（v0.7.0）在本机实测复核**（smoke 通过 / playthrough 13 步逐字吻合 /
> rebirth 仙力 +386、攻击 41→43）。新机上跑出来不一致，基本就是环境问题，别怀疑代码。
> ⚠️ **随机源会推移 rebirth 的「第 2 世变强」数字**：它取决于入世时抽到的灵根/天赋，
> 而抽取用的全局 `G.rng` 序列会被任何**新增的随机消耗**推移。v0.6.0 期间本机实测为 `41→46`，
> 本轮把秘境序列与入口落位改成**独立种子 RNG**（不再消耗全局 rng）后回到 `41→43`。
...
| `portrait.luchen`（主角立绘） | **未出图**，角色面板走程序化半身像；其余 7 张 `portrait.*`（沈伯/刘掌柜/村民/老者/阿阮/杀手/心魔）**已接线** |
...
3. **P4 已收口**（U1/U2/U3/U6 全闭合，见 §8）。剩余按 v3.3 基线推进，按性价比排序：
   ...
   - **M1 内容** / **补素材接线**（`bg.*`、`portrait.luchen`、`obj.rift` / `obj.worldgate`）。
