...
> 最后更新：2026-09-26 · 代码版本 **v0.0.2（天道意志接入）** · 前版 v0.5.2 commit `814d8f2`
> 本次增量：①**20 张副本 Boss 立绘 + 3 张地图 NPC + 7 张人物立绘已出图接线**（见 §8），
> `www/assets/img/` 11 → 41 张、manifest 23 → 53 键；
> ②**四界 28 区域层 + 全部建筑可进 + 副本入口天道随机落位**（见 §8），
> `www/js` 10.5k → **12.8k 行**，新增 `data/regions.js` / `core/regiongen.js` / `core/interiorgen.js`；
> ③**飞升口径修订 + 地狱难度体系**（见 §8）：一世内可连续飞升、**三界地狱碎片齐才开道界**、
> 每界独立难度、跨世永久全属性 +10%；存档 v3 → **v4**。
...
| 代码量 | `www/js` ≈ 10.5k 行；`tools/` ≈ 4.3k 行（无头测试与审查工具） |
| 当前进度 | **M0 主线全通 + 天道意志 + 19境/四界/15副本 + 签名秘术效果 + 20 Boss 立绘 + NPC 精灵/立绘全量替换 + 四界 28 区域层/建筑可进/副本入口随机落位 + 地狱难度体系（三碎片开道界）**（圣术面板% / 神术战斗被动 / 仙术每场一次主动，数值随品阶），道界内容与飞升台未开工 |
| 回归状态 | smoke · playthrough · rebirth · **dungeon-run（凡→灵→仙）** · **secret-test（秘术效果）** 全过；浏览器实测难度页/秘境枢纽/战斗/秘术面板通过 |
...
| `player.js` | 523 | 数值中枢：19境/四界WORLDS/寿元/ascend飞升、`computeStats`（含圣术 `secretPct`）/ `breakState` / `rates` / `xianliOf` |
| `game.js` | 184 | 主控：Canvas 适配、场景路由、主循环、`die()` / `checkAged()` |
| `mapgen.js` | 177 | 地图生成：地面、路、建筑、家具、NPC 落位、可达性（**门格会被标成已占用**，防随机散布把门堵死） |
| `regiongen.js` | 310 | **四界 28 区域生成**：由 `data/regions.js` 的紧凑规格确定性产出完整地图并注册进 `G.Data.maps`；场景工厂 + 建筑门路由；`rollEntrances` 的落位点由这里消费 |
...
| `tiandao.js` | 418 | **天道意志**（v0.0.2 新增）：配置/感应五阶段/世界状态包/OpenAI 兼容调用/JSON 校验/模板兜底/问卦/注视低语/DOM 输入框/**菜单（角色·秘境·界域难度·天道设置）与设置面板**（`G.TianDao`，含 `isMenuOverlay` 路由白名单） |
...
| `death.js` | 250 | 死亡结算：死因/享年/走马灯/仙力明细 → 写入轮回档案 + **天道跨世记忆**（v0.0.2，memory 最近 30 世、watchTotal） |
...
| 工具 | 行数 | 用途 |
|---|---|---|
| `smoke.js` | 1836 | **冒烟测试**：加载全部脚本、走遍所有场景、几十组契约断言（含素材接线、`regions.contract` 区域+建筑+入口落图、`entrances.contract` 落位算法、`hell.contract` 地狱体系、`worlds.panel.contract` 界域难度面板）。改任何东西后第一件事 |
...
3. 当前已落地 **19境/四界/15副本 + 签名秘术效果**（凡界5本→飞升灵界→灵界5本→飞升仙界已无头验证）。剩余按 v3.3 基线推进：
   - **轮回殿 `reincarnation-hall.js` 增飞升台**：四界选择、道之钥匙后显隐藏道界、灵石/道晶兑换、可改难度；
   - **道界道则回廊**：斩三尸/证道/合道、秘术飞升升品（`dungeon.js` 对 dao 目前仅显示“尚未开放”，`SLOT_GL` 无 dao）；
   - 为 20 个 Boss 按《副本Boss形象 v3.3》提示词实际出图（逻辑名见该文，缺图已程序化兜底）；
   - 或 **M1 内容**、**补立绘/背景素材接线**。
