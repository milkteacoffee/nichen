...
> 最后更新：2026-09-26 · 代码版本 **v0.11.3（M1 §11 第 2 步：任务链 m1-1..m1-4）** · 前一里程碑 v0.11.2
> 本次增量（详见 §8 顶部）：
> ①**M1 任务链 m1-1 → m1-4**（`doc/《逆尘》M1剧情与内容设计 v1.0.md` §4）：
>   归镇辨丹 → 外堂探子（探子战 + **抉择 1** 杀/放/交三选）→ 沈伯旧账（赠灵阶功法，**灵根匹配优先**）
>   → 筑基筹备（**两条途径**：刘记订购 1000 灵石 / 沈伯旧方 妖丹×3 + 600 灵石）。
>   新机制：`mapgen` 的 **`condStep` 条件 NPC**、`G.Overlays.choice` 抉择卡、任务面板**滑动窗口**、
>   突破丹别名（炼气圆满要的是**「筑基丹」**）。契约 **22 → 23 条**（新增 `m1.quest.contract`）。
> ②**v0.11.2 玩家截图反馈批**（同一批交付）：HUD/底栏不再整段吞点地、站桩 NPC 不再上下抖、
>   主角四向按「头线」对齐、战斗 30s 思考倒计时 + 按钮去线框 + 功法/道具上浮下拉。
>   契约 20 → 22 条（`ui.fix.contract` + `ui.fix.runtime.contract`）。
> **m1-5..m1-7（血夜 / 筑基心魔 / 离乡）与 bloodhall 据点地图、因果簿、血夜演出 见下一步。**
> v0.11.1（M1 数据层）与更早版本见 §8 下方小节。
...
| `shot.js` | 836 | 73 张场景导出 PNG（`@napi-rs/canvas` 真实光栅化）；支持**帧名过滤**（`shot.js dao_corridor`）。长任务要**后台跑**，前台会被 SIGTERM |
...
| **M1 内容** | **进行中**（v0.11.1 落地**数据层**；v0.11.3 落地**任务链 m1-1..m1-4**：辨丹/探子战+抉择 1/赠功法/筑基丹两途径，含 `condStep` 条件 NPC、`Overlays.choice` 抉择卡、任务面板滑动窗口、`m1.quest.contract`）；**m1-5..m1-7（血夜 / 筑基心魔 / 离乡）/ 血煞据点地图 `bloodhall` / 因果簿 / 血夜演出 未开工** |
