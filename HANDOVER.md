...
> 最后更新：2026-09-26 · 代码版本 **v0.14.0（战斗法力值 + 功法碎片 + 面板卷轴版式）** · 前一里程碑 v0.13.0
> 本次增量（详见 §8 顶部）—— **两处玩法改动 + 一处版式改动**：
> ①**战斗法力值**（用户口径："释放主动技要消耗法力，不然太无敌"）：
>   新增 `st.mpMax`（法力上限，境界成长 + 仙术功法贡献，**不新增第六个来源**）；
>   `G.Player.manaCost(sd, lv) = (10 + (lv-1)×4) × tierCoef` —— **最低 10 点、等级越高耗越多**；
>   `G.Player.MP_REGEN = 5`（每回合回复）；法力不足时技能按钮禁用**并给出原因**（不是只"灰着"）；
>   战斗名牌多一条法力条；角色「属性」页与「加成来源」同步展示。
> ②**功法碎片**：按品阶三种（凡/灵/宝，界→品阶**单调不降**）；副本通关按档次掉
>   （大 3–5 / 小 1–2 / 重复刷减半）；野外刷怪**逐只 6% 几率**掉；10 片在「功法」页**参悟**
>   （优先补未习得，全习得才转 +1 级）。
> ③**六面板改《烟雨江湖》卷轴版式**：**左缘一条竖排标题带**（外框 `FRAME` 与内容区 `P` 分离），
>   底色仍是**云海仙雾**（不是宣纸）；储物格宽 46 → 44 收进新内容区。
> ④契约 **30 → 32 条**（`battle.mana` + `skill.shard`），**反例验证 16 组全部命中**。
> **下一步见 §8 顶部「下一步」。**
> v0.13.0（云海材质）与更早版本见 §8 下方小节。
...
| 当前进度 | **M0 主线全通 + 天道意志（三协议云端模型）+ 19境/四界/15副本 + 签名秘术效果 + 20 Boss 立绘 + NPC 精灵/立绘全量替换 + 主角正面立绘 + 四界 28 区域层/建筑可进/副本入口随机落位 + 地狱难度体系（三碎片开道界）+ 区域裂隙与界门可见 + 副本入口面板 + 轮回殿飞升台（选下一世主界/调界域难度/碎片与称号）+ 飞升·道界·地狱成就 + 常驻底栏六功能 + 每一世经历回溯与设备标识 + 道界道则回廊九关固定试炼（斩三尸→证道→合道）+ 道晶经济 + 秘术飞升升品 + 降世按界起始境界与首区落点** + **野怪收益曲线已校准（各境界刷满 12–42 场，原大罗 60,895 场）** + **区域美术换皮（28 区 28 套配色与装饰物配方，U7 闭合）** + **底栏六面板主动关闭钮** + **角色面板四子页（总览/灵根/属性/境界）** + **M1 数据层（血煞教四敌 + 5 本灵阶功法 + 多段攻击 + 3 张程序化立绘）** + **M1 任务链 m1-1..m1-4（辨丹 / 探子战+抉择 1 / 赠功法 / 筑基丹两途径）** + **四项界面可用性修复（点地不被 HUD 吞 / NPC 不抖 / 四向头线对齐 / 战斗倒计时+下拉）** + **界面批二 8 项（HUD 四格+悬浮说明 / 功法下拉降序 / 境界突破入口 / 灵根九维方块图 / 储物五分类+悬浮 / 左侧任务追踪栏+路引 / 出口云雾传送阵 / 矮墙）** + **Boss 机制与天道心魔对战设计稿 v1.0（只出设计，未改数值）** + **M1 血夜 m1-5..m1-7（血煞据点 bloodhall 连战 / 抉择 2 护镇·护沈伯 / 沈伯燃命·遗言 / 筑基心魔·离乡与 M2 解锁）** + **界面批三（水墨主界面 / 战斗背景八主题 / 追踪栏左缘收缩 / 游戏内版本号归位）** + **UI 材质层改「云海仙雾」（深青紫流云面板 + 云纹角饰 + 云雾玉牌按钮，取代被否掉的宣纸）** + **战斗法力值（上限随境界与仙术功法 / 最低 10 点 / 等级越高耗越多 / 每回合回 5）** + **功法碎片（副本按档次掉 + 野外 6% 掉 + 10 片参悟一本）** + **六面板卷轴版式（左缘竖排标题带，底色仍云海）** |
| 回归状态 | smoke · playthrough · rebirth · **dungeon-run（凡→灵→仙）** · **secret-test（秘术效果）** · **zone-curve（野怪收益曲线验算）** 全过；smoke 共 **32 条契约**（v0.14.0 新增 `battle.mana` + `skill.shard` 两条；v0.13.0 新增 `mist.material` 一条；v0.12.0 新增 `ui.batch3` / `version` / `about.layout` 三条；v0.11.5 新增 `m1.bloodnight` 一条；v0.11.4 新增 `ui.batch2` + `ground.type` 两条；v0.11.3 新增 `m1.quest` 一条；v0.11.2 新增 `ui.fix` + `ui.fix.runtime` 两条；v0.11.1 新增 `m1.data` 一条 + 修掉 `zone.weights` 的时间播种假红；v0.11.0 新增 `region.tint` 一条 + `panels`/`panels.bounds` 两条增强；v0.10.0 批 `zone.curve` / `zone.curve.source` 两条 + 两条运行时差分探针；v0.9.0 批 `dao.trials` / `descend.world` 两条；v0.8.0 批四条同） |
