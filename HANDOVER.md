...
> 最后更新：2026-09-26 · 代码版本 **v0.16.0（建筑换图 + 匾额 + 文字遮挡修复）** · 前一里程碑 v0.15.0
> 本次增量（详见 §8 顶部）—— **一处真缺陷 + 一类新契约能力 + 建筑换素材**：
> ①**文字遮挡**：洞府页副标题被卡片压、末行说明被「坐化」按钮压（顺带抓出设置页一处历史遗留）。
>   ⚠️ **契约原先抓不到**：它只判"文字压在**按钮**上"，而卡片背板走 `G.UI.panel`（`drawImage`）
>   —— **框压字没有任何判据**；且按钮那条阈值是"字高的 40%"，2px 的压边判不出来。
>   新增 **有序层探针**（`layerSpy`）：记录 `G.UI.text`/`textOut` 与 `G.UI.rr`/`G.UI.panel`
>   的**先后序号**，只报"后画的框盖住先画的字"；阈值收到 **1.5px 绝对值**。
>   ⚠️ 探针必须挂在**逻辑入口**（`G.UI.*`）而不是某个 ctx 的 `fillText` ——
>   契约传的 ctx 是它自己造的，另造 ctx 去 patch 会**一条文字都收不到**。
> ②**建筑换文生图素材**：取图键 `struct.<bk>` 优先、退回 `struct.<kind>`
>   （`bk` = 哪家店，`kind` = 画法大类）；**等比内含 + 底部居中**（不拉伸，尺寸从 3×5 到 6×5）；
>   新增 `struct.house` / `struct.apothecary` 两张出图 + 三个占位键（登记进 `SIZES`，共 60 键）。
> ③**建筑匾额**（`explore.js: _drawPlaque`）：**矢量叠加**，绝不烘进素材图
>   （生图里的中文必然乱码；同一张图要复用到不同建筑）。
> ④**四大技艺富化**：设计文档内容升 **v1.1**，补全材料表 / 丹方 / 炼器配方 / 阵法 / 灵兽全表。
> ⑤**「主角的小屋」结论：不需要**（沈家小院 = 镇上住处，洞府 = 修行基地，再加只会重复）。
> ⑥契约仍 **32 条**（新增的是判据不是契约块），**反例验证 3 组全部命中**。
> **下一步见 §8 顶部「下一步」。**
> v0.14.0（法力值 + 碎片 + 卷轴版式）与更早版本见 §8 下方小节。
