...
> 最后更新：2026-09-26 · 代码版本 **v0.15.0（界面重构 + 主动轮回 + 框越界契约）** · 前一里程碑 v0.14.0
> 本次增量（详见 §8 顶部）—— **一处结构性重构 + 三个新入口 + 一条新契约能力**：
> ①**底栏六功能 → 五功能**：**功法 / 秘术并入角色面板的子页**（用户口径"把功法、秘术放到
>   角色面板里面"）→ 角色面板子页 **4 → 6**（总览/灵根/属性/境界/功法/秘术）；
>   **成就移到游戏外**（开局界面）；**新增「洞府」与「地图」**。
>   ⚠️ 功法/秘术仍是**独立的面板实现**，只是从底栏移到子页签（零重构风险）；
>   子页签几何按**当前面板的外框**算（角色页用 `CHAR_PANEL`，功法/秘术页用 `FRAME`）。
> ②**主动轮回**（洞府「坐化」，两步确认）：复用死亡结算，不另写一份。
> ③**成就页移到开局界面 + 富化**：与**设备绑定**（页内显示本机标识）、跨世只发一次；
>   成就 **9 → 18 项**，称号分区展示（原来挤成一行会被截断）。
> ④**破境丹获取指引**：境界页常驻显示"去哪拿"，破境失败的 toast 也带上途径
>   （用户口径："破境失败之后，没有说在哪里可以获取破境丹"）。
> ⑤⚠️ **修一个真缺陷 + 补一类契约能力**：属性页三张属性卡右沿 452 > 面板 436（超出 16px），
>   而**文字契约全绿** —— 因为"文字在卡里居中、卡跑出去了文字还在界内"。
>   新增**框探针**（记录 `G.UI.rr` 的矩形）→ 这类"框越界"从此一次抓全。
> ⑥契约仍 **32 条**：成就页的越界断言并进 `panels.bounds` 一段（它移出底栏清单后就没人管了 —— 覆盖缺口）；**反例验证 4 组全部命中**。
> ⑦出设计文档 `《逆尘》玩法方向与四大技艺设计 v1.0`：**玩法方向定稿走半开放世界**、
>   **主角身份定稿**（现代穿越者 / 无金手指 / 每世随机）、炼丹·炼器·阵法·灵兽的框架与批次。
> **下一步见 §8 顶部「下一步」。**
